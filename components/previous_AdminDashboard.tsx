...
const AdminDashboard: React.FC<{ user: User | null, onLogout: () => void }> = ({ user, onLogout }) => {
            ...
            <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 relative overflow-hidden">
                ...
                <div className="w-full max-w-md bg-white/5 backdrop-blur-2xl p-10 rounded-[3rem] border border-white/10 shadow-2xl relative z-10 animate-in fade-in zoom-in duration-500">
                    <div className="text-center mb-10">
                        <div className="w-20 h-20 bg-indigo-500/20 rounded-3xl mx-auto flex items-center justify-center text-4xl mb-6 border border-indigo-500/30 shadow-lg shadow-indigo-500/20">≡ƒöÉ</div>
    ...
    const allActivities = [
        ...
        ...reviews.map(rev => ({
            ...
            title: 'New Review',
            content: `${rev.menteeName} left a ${rev.rating}Γÿà review for a mentor`,
            status: 'neutral',
            meta: rev.comment
        })),
        ...resources.map(res => ({
                            ...
                            <section className="space-y-12">
                                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                                    <div>
                                        ...
                                        <p className="text-slate-400 font-medium text-lg italic">Database: {dbStatus?.dbName} ΓÇó Sync: {dbStatus?.timestamp ? new Date(dbStatus.timestamp).toLocaleTimeString() : 'N/A'}</p>
                                    </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                                    {[
                                        { label: 'Mentors', value: totalMentors, color: 'rose', trend: dbStatus?.counts?.users ? 'Live Sync' : '+12%', icon: '≡ƒæ¿ΓÇì≡ƒÅ½' },
                                        { label: 'Mentees', value: totalMentees, color: 'blue', trend: 'Verified', icon: '≡ƒæ¿ΓÇì≡ƒÄô' },
                                        { label: 'Sessions', value: totalSessions, color: 'purple', trend: 'DB Checked', icon: '≡ƒôà' },
                                        { label: 'Resources', value: resources.length, color: 'amber', trend: 'Active', icon: '≡ƒôÜ' }
                                    ].map(stat => (
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                                            ...
                                            {allActivities.slice(0, 5).map((act, i) => (
                                                <div key={act.id + i} className="flex gap-4 items-start group">
                                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border border-white/5 transition-colors ${
                                                        ...
                                                        {act.type === 'request' ? '≡ƒñ¥' : act.type === 'review' ? 'Γ¡É' : '≡ƒôª'}
                                                    </div>
                                                    <div className="flex-1">
                                                        ...
                                                        <p className="text-[10px] text-slate-600 font-bold mt-2 uppercase">{act.date.toLocaleDateString()} ΓÇó {act.date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                                                    </div>
                                                </div>
                                            ))}
                                    ...
                                    <div className="bg-white/[0.02] border border-white/5 rounded-[2.5rem] p-8 flex flex-col justify-center items-center text-center">
                                        <div className="w-20 h-20 bg-emerald-500/10 rounded-3xl flex items-center justify-center text-4xl mb-6 border border-emerald-500/20 shadow-2xl shadow-emerald-500/10">≡ƒôê</div>
                                    ...
                                    {allActivities.map((act, i) => (
                                        <div key={act.id + i} className="relative flex gap-8 items-start bg-white/[0.02] hover:bg-white/[0.04] p-6 rounded-3xl border border-white/5 transition-all group">
                                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 border-2 z-10 transition-transform group-hover:scale-110 ${
                                                ...
                                                <span className="text-2xl">{act.type === 'request' ? '≡ƒñ¥' : act.type === 'review' ? 'Γ¡É' : '≡ƒôª'}</span>
                                            </div>
                                            <div className="flex-1">
                                    ...
                                    {userTypeTab === 'mentor' ? (
                                        ...
                                        <div className="space-y-4 animate-in fade-in slide-in-from-left-4 duration-500">
                                            <div className="flex items-center gap-4 px-2">
                                                <div className="w-8 h-8 bg-indigo-500/10 rounded-lg flex items-center justify-center text-indigo-400">≡ƒæ¿ΓÇì≡ƒÅ½</div>
                                        ...
                                        <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-500">
                                            <div className="flex items-center gap-4 px-2">
                                                <div className="w-8 h-8 bg-rose-500/10 rounded-lg flex items-center justify-center text-rose-400">≡ƒæ¿ΓÇì≡ƒÄô</div>
